...
{
    if ($status == 3) {
        return '<span class="badge bg-danger">Dữ liệu đầu vào không hợp lệ</span>';
    } elseif ($status == 0) {
        return '<span class="badge bg-success">Hoàn tất</span>';
    } elseif ($status == 2) {
        return '<span class="badge bg-danger">Hết hạn (hoàn tiền)</span>';
    } elseif ($status == 1) {
...
function card24h($telco, $amount, $serial, $pin, $trans_id){
    ...
    $url = 'https://gachthe1s.com/chargingws/v2?sign='.md5($partner_key.$pin.$serial).'&telco='.$telco.'&code='.$pin.'&serial='.$serial.'&amount='.$amount.'&request_id='.$trans_id.'&partner_id='.$partner_id.'&command=charging';
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $data = curl_exec($ch);
